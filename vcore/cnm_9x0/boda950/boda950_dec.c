...
static void *boda950_cnm_dec_instance = NULL;

enum vcore_dec_ret _boda950_dec_open(void **vcore_id,
...
{
	void *cnm_id = boda950_get_cnm_core_instance();

	return cnm_dec_open(vcore_id,
						cnm_id, boda950_cnm_dec_instance,
						codec_type,
...
{
	if (boda950_cnm_dec_instance)
		cnm_dec_broadcast(cnm_id, boda950_cnm_dec_instance, vcore_id);
}

void boda950_dec_report_reset(void)
{
	cnm_dec_report_reset(boda950_cnm_dec_instance);
}

void boda950_dec_init(struct vcore_dec_ops *ops)
{
	ops->open = _boda950_dec_open;
	...
	if (boda950_cnm_dec_instance == NULL)
		boda950_cnm_dec_instance = cnm_dec_init();
}
